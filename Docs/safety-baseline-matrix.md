...
| TODAY-02 | Every actionable rule remains visible; an unresolved rule owning a rule lock remains the hero until completed or slipped | `testActionableMorningSunStaysVisibleWhileReadyForSleepIsForTonight`, `testMorningSunVisibleThroughoutConfiguredWindow`, `testExpiredMorningSunStaysVisibleAndOwnsHeroWhenItIsLocking`, `testLockedMorningSunStaysVisibleWhenReadyForSleepIsAlsoVisible` | `testSleepWindowDoesNotShowMorningSunBeforeItIsAvailable`; `testLockedMorningSunCannotBeDisplacedByLaterCutoffCards` prevents a lock/card mismatch | Automated |
| TODAY-03 | Hold-to-confirm completes the visible hero | `HoldToConfirmUITests.testShortHoldDoesNotCompleteAndFullHoldCompletesHero` performs both gestures; `testHoldConfirmUITestFixtureShowsActionableHero` proves the state transition | The UI test proves a short hold does not complete and a full three-second hold does; D-02 remains the physical-device interaction check | Automated + required device confirmation |
...
| RULES-01 | Rules screen shows and atomically edits current rule/window configuration | `testCommittingAfternoonRuleTimePreservesPMAndDoesNotLockFromIntermediateAM`; D-03 | Intermediate hour/minute/AM-PM picker values cannot persist, reschedule, or shield; Cancel makes no change; locked/disabled controls cannot mutate state | Automated state regression + required device UI confirmation |
...
| D-02 Today interactions | Exercise visible hero complete, short hold/tap, slip, cooldown, late completion, final in-bed confirmation, and all-clear. During each rule lock, verify the banner and hero name the same rule. Only the full hold completes; each transition occurs once. |
| D-03 Rules editing | Edit every surfaced rule time/grace and sleep window; relaunch and verify persistence. While editing an afternoon time, move another wheel before switching AM to PM and verify nothing persists, reschedules, or locks until Save; verify Cancel changes nothing and Save preserves PM. Move a future rule to the current minute and verify the app remains unshielded for its full grace, then locks exactly at grace end if unresolved. During any locked state, verify controls marked unavailable cannot change configuration. |
...
| --- | --- | --- |
| Compile | iPhone 17 Pro, iOS 26.5 simulator; 2026-08-10 | Passed |
| Release compile | Generic iOS device, unsigned Release configuration; 2026-08-10 | Passed |
| Unit | 62 `LullTests`; iPhone 17 Pro, iOS 26.5 simulator; 2026-08-10 | Passed |
| UI smoke | 4 tests in `NavigationSmokeUITests` and `HoldToConfirmUITests`; iPhone 17 Pro, iOS 26.5 simulator; 2026-08-10 | Passed |
